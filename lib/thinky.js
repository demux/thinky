...
Thinky.prototype.createModelFromClass = function(cls) {
  const model = this.createModel(cls.name, cls.schema, cls.options);

  model.on('_docInit', (doc) => {
    util.copyOwnProperties(cls.prototype, doc, {
      exclude: ['constructor']
    });
    if(doc.init) {
  ...
  util.copyOwnProperties(cls, model, {
