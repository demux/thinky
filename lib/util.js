...
/**
 * Copy own property descriptors of one object to another, using
 * `Object.getOwnPropertyDescriptor` and `Object.defineProperty`.
 *
 * Note: Unlike `Object.assign`, `assignPropertyDescriptors` will not overwrite.
 *
 * In it's simplest form, this would be:
 * @example
 * function assignPropertyDescriptors(to, from) {
 *   return Object.defineProperties(to, Object.getOwnPropertyDescriptors(from))
 * }
 *
 * @param {Object} to - Receives property descriptors.
 * @param {Object} from - Supplies own property descriptors.
 * @param {Object} options - Extra options. Arguments:
 * - exclude {Array} A list of keys that should not be copied.
 */
function assignPropertyDescriptors(to, from, options) {
  ...
  for(let i in keys) {
    const k = keys[i];
    if(options.exclude.indexOf(k) === -1 && !Object.hasOwnProperty(to, k)) {
      Object.defineProperty(to, k, Object.getOwnPropertyDescriptor(from, k));
    }
  }
}
util.assignPropertyDescriptors = assignPropertyDescriptors;
